    ...
    <div className="p-[8.5px] border-b border-gray-700 bg-gray-900">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {isGroupChat ? (
            ...
            <div className="flex items-center gap-2">
              <div className={`w-1.5 h-1.5 rounded-full ${onlineUsers.includes(user?._id || '') ? "bg-green-500" : "bg-gray-500"}`}></div>
              <span className={`text-xs ${onlineUsers.includes(user?._id || '') ? "text-green-500" : "text-gray-500"}`}>
      ...
      {isGroupChat && showMembers && (
        <div className="mt-3 pt-3 border-t border-gray-700">
          <p className="text-xs font-semibold text-gray-400 mb-2">Group Members:</p>
          <div className="space-y-1 max-h-32 overflow-y-auto">
            {groupMembers.map((member) => (
              <div key={member._id} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${onlineUsers.includes(member._id) ? 'bg-green-500' : 'bg-gray-500'}`}></div>
